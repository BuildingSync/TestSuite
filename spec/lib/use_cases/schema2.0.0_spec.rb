...
describe 'A PROPER spec/use_cases/schema2.0.0/L000_Simulation_BRICR.sch' do
  before(:all) do
    sch_path = File.join(File.dirname(__FILE__), '../../use_cases/schema2.0.0/L000_Simulation_BRICR.sch')
    sch_file = Nokogiri::XML File.open sch_path
    @stron = SchematronNokogiri::Schema.new sch_file

  end

  it "Should issue three 'errors' at the [INFO] level when run against spec/use_cases/schema2.0.0/examples/L000_Simulation_BRICR_01.xml" do
    xml_path = File.join(File.dirname(__FILE__), '../../use_cases/schema2.0.0/examples/L000_Simulation_BRICR_01.xml')
  ...
  it "Should issue three 'errors' at the [INFO] level when run against spec/use_cases/schema2.0.0/examples/L000_Simulation_BRICR_02.xml" do
    xml_path = File.join(File.dirname(__FILE__), '../../use_cases/schema2.0.0/examples/L000_Simulation_BRICR_02.xml')
    doc = Nokogiri::XML File.open xml_path # create a Nokogiri::XML::Document
