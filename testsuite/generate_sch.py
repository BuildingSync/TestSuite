...
def add_assert_description(assert_elem, description):
    ...
    for match in matches:
        ...
        inserted_elem = etree.SubElement(
            assert_elem, qname(some_elem.tag), **{k: v for (k, v) in some_elem.items()}
        )
        inserted_elements.append({"element": inserted_elem, "text_span": match.span()})
...
def generate_sch(
    ...
    for phase in sch_dict["phases"]:
        phase_elem = etree.SubElement(
            root, qname("phase"), id=phase["id"], see=phase["see"]
        )
