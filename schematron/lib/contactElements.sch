...
-->
  <pattern abstract="true" id="con.nameEmailPhone">
    <rule context="$parent">
      <assert test="auc:ContactName" role="ERROR">element 'auc:ContactName' is REQUIRED EXACTLY ONCE for: '<name/>'</assert>
      <assert test="count(auc:ContactEmailAddresses//auc:EmailAddress[text()]) &gt;= 1" role="ERROR">element 'auc:EmailAddress' within element 'auc:ContactEmailAddresses/auc:ContactEmailAddress' is REQUIRED AT LEAST ONCE for: '<name/>'</assert>
      <assert test="count(auc:ContactTelephoneNumbers//auc:TelephoneNumber[text()]) &gt;= 1" role="ERROR">element 'auc:TelephoneNumber' within element 'auc:ContactTelephoneNumbers/auc:ContactTelephoneNumber' is REQUIRED AT LEAST ONCE for: '<name/>'</assert>
    </rule>
  </pattern>
  <!--
...
-->
  <pattern abstract="true" id="con.notAuditorAndOwner">
    <rule context="$parent">
      ...
      <assert test="$numOwners * $numAuditors = 0" role="ERROR">element 'auc:ContactRole' MUST NOT HAVE values 'Energy Auditor' and 'Owner' within 'auc:ContactRoles' for: '<name/>'</assert>
    </rule>
  </pattern>
</schema>
